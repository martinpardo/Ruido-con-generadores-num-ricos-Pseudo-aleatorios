...
void setup(){
  ...
  svf.setResonance(25); // 0 to 255, 0 is the "sharp" end
...
void updateControl(){
  del_samps = 128+aNoise.next();
}


AudioOutput updateAudio(){
  
  int input = aDel.next(aNoise.next()>> 1, del_samps); // shift down (ie. fast /) to avoid distortion with extreme resonant filter setting
